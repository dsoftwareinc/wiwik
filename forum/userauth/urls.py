...
urlpatterns = [
    ...
    path('staff-deactivate/<str:username>/', views.view_deactivate_user, name='deactivate_user'),
    ...
    re_path(r'^activate/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]+)/$',
            views.view_activate, name='activate'),
    path('', views.view_users, name='list'),
    path('query/', views.view_users_query, name='list_query'),
    re_path(r'^unsubscribe/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]+)/$',
            views.view_unsubscribe, name='unsubscribe'),
