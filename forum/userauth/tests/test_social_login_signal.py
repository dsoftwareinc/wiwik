...
class UserAuthLoginTest(UserAuthTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_no_social = ForumUser.objects.create_user(
            cls.username_no_social, f'{cls.username_no_social}@a.com', cls.password,
            name=cls.username_no_social)
        cls.extra_data = {"id": "100466292043055079210",
                          ...
                          "locale": "en", "hd": "backbase.com"}
        cls.socialaccount1 = SocialAccount.objects.create(
            user=cls.user1, provider='google', uid=cls.extra_data['id'], extra_data=cls.extra_data)

        cls.socialaccount2 = SocialAccount.objects.create(
            user=cls.user1, provider='facebook', uid=cls.extra_data['id'], extra_data=cls.extra_data)

    def test_user_signed_up__green(self):
        # arrange
        social_login = SocialLogin(self.user1, self.socialaccount1)
        # act
        populate_profile(social_login, self.user1, )
        # assert
    ...
    def test_user_signed_up__no_socialaccount__should_not_populate(self):
        ...
        # act
        populate_profile(social_login, self.user_no_social, )
        # assert
        ...
        self.assertIn(os.path.join(settings.MEDIA_ROOT, 'default_pics/default_image.jpg'),
    ...
    def test_user_signed_up__bad_socialaccount__should_not_populate(self):
        ...
        # act
        populate_profile(social_login, self.user2, )
        # assert
        self.user2.refresh_from_db()
        self.assertEqual(f'{self.username2}@a.com', self.user2.email)
        self.assertIsNone(self.user2.name)
        self.assertIn(os.path.join(settings.MEDIA_ROOT, 'default_pics/default_image.jpg'), self.user2.profile_pic.path)
    ...
    def test_user_logged_in__green(self):
        ...
        # act
        fill_missing_data_in_profile(social_login, self.user1, )
        # assert
        self.user1.refresh_from_db()
    ...
    def test_user_logged_in__bad_socialaccount__should_not_populate(self):
        ...
        # old_path = self.user1.profile_pic.path
        # act
        fill_missing_data_in_profile(social_login, self.user2, )
        # assert
        self.user2.refresh_from_db()
        self.assertEqual(f'{self.username2}@a.com', self.user2.email)
        self.assertIsNone(self.user2.name)
