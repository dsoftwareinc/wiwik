...
class ForumUser(AbstractUser):
    ...
    github_handle = models.CharField(
        max_length=39, default=None, null=True, blank=True,
    ...
    slack_userid = models.CharField(
        ...
        help_text='Slack user id', unique=True)
    ...
    last_email_datetime = models.DateTimeField(null=True, blank=True,
                                               help_text='Date+Time of email sent to user', )
    bronze_badges = models.IntegerField(
        default=0,
        help_text='Number of bronze badges the user has')
    silver_badges = models.IntegerField(
        default=0,
        help_text='Number of silver badges the user has')
    gold_badges = models.IntegerField(
        default=0,
        help_text='Number of gold badges the user has')
    next_badge = models.ForeignKey(
        'badges.Badge', blank=True, null=True,
    ...
    people_reached = models.IntegerField(
        default=0,
        help_text='Number of views posts user created had')
    posts_edited = models.IntegerField(
        default=0,
        help_text='Number of posts user edited')
    ...
    def has_perm(self, perm, obj=None):
        ...
        if self.is_superuser:
            return True
        if perm in {
            'question_edit', 'question_delete',
            'article_edit', 'article_delete', 'article_create',
            'answer_edit', 'answer_delete',
            'comment_delete',
            'tag_edit', 'synonym_approve',
        }:
