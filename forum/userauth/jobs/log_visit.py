...
def log_request(user: ForumUser, client_ip: str,
    ...
    if (UserVisit.objects
            .filter(user=user, visit_date=time.date(),
                    country=country_name, city=city_name)
            .exists()):
        return
    consecutive_days = (UserVisit.objects
                        .filter(user=user,
                                visit_date=time.date() - timedelta(days=1))
                        .order_by('-visit_date')
                        .values_list('consecutive_days', flat=True)
                        .first() or 0) + 1
    total_days = (UserVisit.objects.filter(user=user)
                  .order_by('-total_days')
                  .values_list('total_days', flat=True)
                  .first() or 0) + 1
    max_consecutive_days = max(UserVisit.objects
                               .filter(user=user)
                               .order_by('-consecutive_days')
                               .values_list('consecutive_days', flat=True)
                               .first() or 0, consecutive_days)
    UserVisit.objects.create(
        user=user, ip_addr=client_ip, visit_date=time.date(),
        country=country_name, city=city_name,
