...
def get_created_at(obj):
    if (obj.created_at.tzinfo is not None
...
def unseen_reputation_sum(u: ForumUser):
    return (u.reputation_votes
            .filter(seen__isnull=True, reputation_change__isnull=False)
            .aggregate(Sum('reputation_change'))
            .get('reputation_change__sum', 0) or 0
            )
