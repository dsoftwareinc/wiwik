...
class TestEditTag(TagsApiTestCase):
    ...
    def test_edit_tag_get__not_logged_in__redirect(self):
        ...
        assert_url_in_chain(
            res, reverse('userauth:login') + '?next=' + reverse('tags:edit', args=[self.tags[0].tag_word, ]))
    ...
    def test_edit_tag_post__green(self):
        ...
        # assert
        self.assertEqual(200, res.status_code)
        assert_url_in_chain(res, reverse('tags:info', args=[self.tags[0].tag_word, ]))
    ...
    def test_edit_tag_post__twice_by_same_user__should_have_one_edit(self):
        ...
        # act
        res = self.client.edit_post(self.tags[0].tag_word, self.new_description, self.new_wiki + '2', self.summary)
        # assert
        self.assertEqual(200, res.status_code)
        assert_url_in_chain(res,
                            reverse('tags:info', args=[self.tags[0].tag_word, ]))
        assert_message_in_response(res, f'Tag {self.tags[0].tag_word} edited successfully')
        tag = Tag.objects.get(tag_word=self.tags[0].tag_word)
        tag.refresh_from_db()
        self.assertEqual(self.new_description, tag.description)
    ...
    def test_edit_tag_post__wiki_too_short__should_fail(self):
        ...
        assert_url_in_chain(res,
                            reverse('tags:info', args=[self.tags[0].tag_word, ]))
        assert_message_in_response(res,
    ...
    def test_edit_tag_post__description_too_long__should_fail(self):
        self.client.login(self.username, self.password)
        # act
        res = self.client.edit_post(self.tags[0].tag_word,
                                    self.new_description + 'x' * settings.MAX_TAG_DESCRIPTION_LENGTH,
                                    self.new_wiki, self.summary)
        # assert
        self.assertEqual(200, res.status_code)
        assert_url_in_chain(
            res,
    ...
    def test_edit_tag_post__wiki_too_long__should_fail(self):
        ...
        res = self.client.edit_post(
            tag_word, self.new_description, 'very long' + 'x' * settings.MAX_TAG_WIKI_LENGTH, self.summary)
        # assert
        self.assertEqual(200, res.status_code)
        assert_url_in_chain(res,
                            reverse('tags:info', args=[self.tags[0].tag_word, ]))
        assert_message_in_response(res,
                                   'Error: Wiki content too long')
        tag = Tag.objects.get(tag_word=self.tags[0].tag_word)
        tag.refresh_from_db()
        self.assertNotEquals(self.new_description, tag.description)
        self.assertNotEquals(self.new_wiki, tag.wiki)
    ...
    def test_edit_tag_post__summary_too_long__should_fail(self):
        self.client.login(self.username, self.password)
        # act
        res = self.client.edit_post(self.tags[0].tag_word, self.new_description, self.new_wiki,
