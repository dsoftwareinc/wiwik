...
class TestEditTag(TagsApiTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.new_description = "New description for tag with 20 chars" + "X" * max(
            config.MIN_TAG_DESCRIPTION_LENGTH - 37, 0)
        cls.new_wiki = "New wiki for tag with more than 20 chars"
        cls.summary = "Edit summary" + "X" * max(config.MIN_TAG_EDIT_SUMMARY_LENGTH - 12, 0)
