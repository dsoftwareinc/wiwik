...
def create_new_suggestion(request, synonym_name: str, tag_word: str):
    ...
    if Synonym.objects.filter(name=synonym_name).exists():
        messages.warning(request, 'Synonym with this name already suggested for another tag, aborting')
        return
...
def _get_order_by_from_request(request):
    try:
        ...
        order_by_num = -5
    order_by = (ORDER_BY_MAP.get(order_by_num, 'created_at') if order_by_num > 0
...
def view_approve_synonym(request, synonym_pk: int):
    ...
    if user.can_approve_synonym:
        ...
        logger.warning(f'user {request.user.username} tried to approve synonym '
