...
class QuestionInline(admin.TabularInline):
    ...
    ordering = ('-created_at',)
    can_delete = False
    fields = ('space', 'title', 'author', 'created_at',)
...
class SpaceAdmin(admin.ModelAdmin):
    save_on_top = True
    list_display = (
        'id', 'short_name', 'name', 'restricted', 'start_date',
        'end_date', 'space_active', 'properties_str', 'author', 'created_at',)
    ...
    inlines = [SpaceToPropertyInline, SpaceMemberInline, QuestionInline, ]
    fields = (('short_name', 'name', 'logo', 'author'),
              ('start_date', 'end_date',),
              ('page',),)
...
class SpacePropertyAdmin(admin.ModelAdmin):
    list_display = ('id', 'name', 'author', 'created_at', 'spaces_count',)
    list_display_links = ('id', 'name',)
    raw_id_fields = ('author',)
    search_fields = ('name', 'author__username', 'author__email',)
    list_filter = (('created_at', DateRangeFilter),)
    readonly_fields = ('created_at',)
    inlines = [SpaceToPropertyInline, ]

    def get_queryset(self, request):
        qs = super(SpacePropertyAdmin, self).get_queryset(request)
        qs = qs.annotate(spaces_count=Count('spaces'), )
        return qs
