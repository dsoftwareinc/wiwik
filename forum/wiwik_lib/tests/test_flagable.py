...
class TestFlagAnswerView(FlagApiTestCase):
    ...
    def test_flag_answer__green(self, start_job: mock.MagicMock):
        ...
        assert_url_in_chain(res, reverse('forum:thread', args=[self.question_pk]) + f'#answer_{self.answer_pk}')
        start_job.assert_has_calls([
            mock.call(jobs.notify_user_email, self.users[2], mock.ANY, mock.ANY, mock.ANY, True),
            mock.call(slack_api.slack_post_im_message_to_email, mock.ANY, self.users[2].email),
        ])
...
class FlagsAdminTest(FlagApiTestCase):
    ...
    def setUpClass(cls):
        ...
        Flag.objects.create(
            user=cls.users[1], flag_type='rude',
