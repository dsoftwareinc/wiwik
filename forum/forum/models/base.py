    ...
    def with_documents(self):
        if self.support_postgres_fts:
            vector = (SearchVector("title", weight='A')
                      ...
                      + SearchVector(StringAgg('answer__content', delimiter='\n', output_field=TextField()), weight='D')
                      )
        else:
            vector = F('title')
...
class Question(VotableUserInput, Flaggable):
    """
    Class to represent a question in the forum
    """
    ...
    views = models.IntegerField(
        default=0, help_text='Number of views for post',
    )
    ...
    title = models.CharField(
        max_length=255, help_text='Post title',
    )
    tags = models.ManyToManyField(
        Tag, help_text='Tags post is asscociated with',
    )

    is_anonymous = models.BooleanField(
        default=False, help_text='Is post anonymous?',
    )
    has_accepted_answer = models.BooleanField(
        default=False, help_text='Does post have an accepted answer?',
    )
    space = models.ForeignKey(
        Space, on_delete=models.SET_NULL, null=True, blank=True,
        help_text='Space post is associated with',
    )
    source = models.CharField(
        max_length=10, choices=SOURCES, null=True, blank=True,
