...
def initial_query(qs: QuerySet, query: str):
    ...
    for qpart in query_parts:
        if len(qpart) <= 1:
            ...
            qs = qs.filter(
                space__short_name__iexact=space_name
            )
        else:
...
def postgres_query_method(qs: QuerySet, initial_q: str) -> QuerySet:
    ...
    res_qs = (qs
              .annotate(relevance=search_rank)
    ...
    if res_qs.count() == 0:
        ...
        res_qs = (qs
                  .annotate(title_distance=TrigramDistance('title', query))
...
def meilisearchmethod():
    ...
    def meilisearch(qs: QuerySet, initial_q: str):
        ...
        if not query:
            return qs.annotate(relevance=Value(1))
        meilisearch_response = index.search(query, {
        ...
        if len(question_ids) > 0 and _postgres_enabled():
            qs = (qs.annotate(ordering=ArrayPosition(question_ids, F('id'), output_field=BigIntegerField()))
                  .order_by('ordering'))
        return qs
