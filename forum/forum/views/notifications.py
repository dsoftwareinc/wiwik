...
def notify_tag_followers_new_question(
        originator: AbstractUser, tag_words: Set[str], q: models.Question) -> None:
    """
    ...
    slack_msg = slack_template.render(context={
        'q': q,
    ...
    for user in emails_dict:
        html = template.render(
            context={'tag_word': emails_dict[user], **context}
        )
        jobs.start_job(jobs.notify_user_email, user,
                       f'Activity in tag {emails_dict[user]}',
                       activity_str, html, False)


def notify_question_changes(originator: AbstractUser, q: models.Question,
...
def notify_answer_changes(originator: AbstractUser, a: models.Answer, old_content: str):
    ...
    activity_str = (f'Answer to "{q.title}" by {a.author} was updated' +
                    f"link: {model_url}\n")
...
def notify_new_comment(
    ...
    user_display_name = _get_user_display_name(originator)
    activity_str = (f'New comment to "{parent.get_question().title}" {parent.get_model()} '
