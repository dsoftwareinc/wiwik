...
def view_users_autocomplete(request):
    ...
    if query is not None:
        qs = (
            qs.filter(Q(username__icontains=query) | Q(name__icontains=query) | Q(email__icontains=query))
...
def view_get_users_data(request):
    ...
    qs = ForumUser.objects.all().filter(Q(username__in=usernames)).exclude(is_staff=True).exclude(is_active=False)
    results = list(qs.values("username", "name", "email", "profile_pic"))[:10]
    for result in results:
        if result["profile_pic"][0] != "/":
            result["profile_pic"] = "/" + result["profile_pic"]
...
def view_invite_to_question(request, question_pk: int):
    ...
    invitees = ForumUser.objects.filter(username__in=invitee_usernames).exclude(username=question.author.username)
