...
def view_acceptanswer(request, question_pk: int, answer_pk: int):
    ...
    if answer.question != question:
        logger.warning(
            f"Trying to approve answer {answer_pk} for question {question_pk} but answer is for a different question"
        )
        return redirect("forum:thread", pk=question_pk)
    if question.author != user and not user.is_staff:
        logger.warning(f"user {user.username} trying to approve answer on question {question_pk} without permission")
        return redirect("forum:thread", pk=question_pk)
    ...
    logger.info(f"user {user.username} approved answer {answer_pk} on question {question_pk}")
