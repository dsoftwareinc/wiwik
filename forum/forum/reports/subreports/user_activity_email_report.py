...
class UserData(object):
    def __init__(self, username: str, reputation: int,
                 num_questions: int = 0, num_answers: int = 0,
                 num_question_comments=0, num_answer_comments=0,
                 num_visits=0, search_count=0):
    ...
    def valid(self) -> bool:
        return (self.num_questions > 0 or self.num_answers > 0
                or self.num_question_comments > 0 or self.num_answer_comments > 0
...
def user_activity_email_report(from_date: date):
    users_dict = dict()
    users_visited = list(ForumUser.objects
                         .filter(uservisit__visit_date__gte=from_date)
                         .annotate(num_visits=Count('uservisit'))
                         .filter(num_visits__gt=0)
                         .values_list('username', 'num_visits',
    ...
    users_questions_list = list(ForumUser.objects
                                .filter(question__created_at__gte=from_date)
    ...
    users_answers_list = list(ForumUser.objects
                              .filter(answer__created_at__gte=from_date)
    ...
    users_qcomments_list = list(ForumUser.objects
                                .filter(questioncomment__created_at__gte=from_date)
    ...
    users_acomments_list = list(ForumUser.objects
                                .filter(answercomment__created_at__gte=from_date)
