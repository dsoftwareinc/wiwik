...
from forum.jobs import warn_users_loosing_moderator_status, update_moderator_status_for_users
...
class TestModeratorJobs(ForumApiTestCase):
    ...
    def setUpClass(cls):
        ...
        UserVisit.objects.create(
            user=cls.users[0], visit_date='2021-10-12', consecutive_days=1, )
        UserVisit.objects.create(
            user=cls.users[0], visit_date='2021-10-10', consecutive_days=1, )
        UserVisit.objects.create(
            user=cls.users[2], visit_date='2021-10-12', consecutive_days=1, )
        UserVisit.objects.create(
            user=cls.users[2], visit_date='2021-10-10', consecutive_days=1, )
        UserVisit.objects.create(
            user=cls.users[2], visit_date='2021-10-09', consecutive_days=1, )
