...
class TestModeratorJobs(ForumApiTestCase):
    ...
    def test_warn_users_loosing_moderator_status__green(
        self, notify_user_email: mock.MagicMock
    ):
        # arrange
        ...
        warn_users_loosing_moderator_status()
        # assert
        notify_user_email.assert_called_with(
            self.users[1], subject, subject, mock.ANY, True
        )
        assert_not_called_with(
            notify_user_email, self.users[0], mock.ANY, mock.ANY, mock.ANY, mock.ANY
        )
        assert_not_called_with(
            notify_user_email, self.users[2], mock.ANY, mock.ANY, mock.ANY, mock.ANY
        )

    @mock.patch("datetime.date", FakeDate)
    @mock.patch("forum.jobs.notify_user.notify_user_email")
    def test_update_moderator_status_for_users__green(
        self, notify_user_email: mock.MagicMock
    ):
        # arrange
        ...
        notify_user_email.assert_has_calls(
            [
                mock.call(
                    self.users[1], revoke_subject, revoke_subject, mock.ANY, True
                ),
