...
class TestReportJobs(TestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.users = [ForumUser.objects.create_user(username, f'{username}@a.com', cls.password)
                     for username in cls.usernames]
        cls.old_question = utils.create_question(cls.users[0], cls.title, cls.question_content, ','.join(cls.tags),
    ...
    def test_send_weekly_digest_for_users__green(self, notify_user: mock.MagicMock, single_digest: mock.MagicMock, ):
        # arrange
    ...
    @mock.patch('forum.jobs.notify_user.notify_user_email')
    def test_send_weekly_digest_for_users__only_users_with_data__should_be_2(self, notify_user: mock.MagicMock, ):
        # arrange
        subject = f'Weekly digest on tags you are following, activity since {self.fromdate.date()}'
        # act
        jobs.send_weekly_digest_for_users()
