...
class TestSlackApi(TestCase):
    ...
    def test_slack_post_channel_message__green(self):
        ...
        # assert
        slack_api.slack_client.chat_postMessage \
            .assert_called_once_with(channel=self.channel, thread_ts=None, text=self.text, blocks=mock.ANY, mrkdwn=True)
    ...
    def test_slack_post_channel_message__exception(self):
        ...
        slack_api.slack_client.chat_postMessage.assert_called_once_with(
            channel=self.channel, thread_ts=None, text=self.text, blocks=mock.ANY, mrkdwn=True)

    def test_slack_post_im_message__green(self):
        # arrange
        user_id = 'id'
        slack_api.slack_client = MagicMock()
        slack_api.slack_client.users_lookupByEmail.return_value = {'ok': True, 'user': {'id': user_id}}
        # act
