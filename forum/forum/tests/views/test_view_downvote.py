...
class TestDownvoteView(ForumApiTestCase):
    ...
    def test_downvote_question__green(self):
        ...
        self.assertEqual(settings.DOWNVOTE_CHANGE, item.reputation_change)
    ...
    def test_downvote_answer__green(self):
        ...
        # assert
        assert_url_in_chain(
            res, reverse('forum:thread', args=[self.question.pk]) + f'#answer_{self.answer_diff_user.pk}')
    ...
    def test_downvote_answer__when_already_downvoted__delete_downvote(self):
        ...
        a = (models.Answer.objects.filter(id=self.answer_diff_user.pk)
             ...
             .first())
        self.assertEqual(
            previous_reputation - settings.DOWNVOTE_CHANGE,
            a.author.reputation_score)
