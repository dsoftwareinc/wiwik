...
class TestPostCommentsPartialView(ForumApiTestCase):
    ...
    def test_comments_partial_view__question_max_comments_not_reached(self):
        ...
        self.assertEqual(1, len(soup.find_all('form', {'class': 'form-add-comment', 'model': 'question',
                                                       'pk': self.question.pk})))
        self.assertEqual(1, len(soup.find_all('div', {'class': 'add-comment-button', 'model': 'question',
                                                      'pk': self.question.pk})))
    ...
    def test_comments_partial_view__answer_max_comments_not_reached(self):
        ...
        self.assertNotContains(res, 'Maximum number of comments reached')
        soup = BeautifulSoup(res.content, 'html.parser')
        self.assertEqual(1, len(soup.find_all('form', {'class': 'form-add-comment', 'model': 'answer',
                                                       'pk': self.question.pk})))
        self.assertEqual(1, len(soup.find_all('div', {'class': 'add-comment-button', 'model': 'answer',
                                                      'pk': self.question.pk})))
