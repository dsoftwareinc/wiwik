...
def revoke_moderator(user: ForumUser, num_visits: int, last_month: datetime):
    ...
    html = template.render(context={'last_month': last_month,
...
def send_warning_loosing_moderator_status_to_user(user: ForumUser, num_visits: int, bom: datetime) -> None:
    ...
    moderator_revoke_warning = template.render(context={
        'num_visits': num_visits,
    ...
    html = generate_report_html(
        '',
        [moderator_revoke_warning, questions_report, ],
        unsubscribe_link,
    )
...
def warn_users_loosing_moderator_status():
    ...
    moderator_qs = (ForumUser.objects
                    .filter(is_moderator=True, is_superuser=False, is_staff=False, is_active=True)
                    .annotate(num_visits=Count('uservisit',
                                               filter=Q(uservisit__visit_date__gte=beginning_of_month))))
