...
def weekly_digest_to_user(
    ...
    digest_report = recent_questions_email_report(
        fromdate, tag_words, skip_if_empty=True
    )
    unanswered_questions = old_unanswered_questions_email_report(
        tag_words, skip_if_empty=True
    )
    if digest_report == "":
        logger.debug(
...
def send_weekly_digest_for_users():
    ...
    user_qs = ForumUser.objects.filter(
        is_superuser=False, is_active=True, email_notifications=True
    )
    logger.debug(f"Sending weekly digest to {len(user_qs)} users")
    subject = (
        f"Weekly digest on tags you are following, activity since {fromdate.date()}"
    )
    for user in user_qs:
